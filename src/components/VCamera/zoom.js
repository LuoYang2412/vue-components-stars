...
export default class zoom {
  ...
  zoomElement(el) {
    ...
    hammer.on('pinchmove pinchstart pinchin pinchout', (e) => {
      ...
      this.drawX = -x / currentScale
      this.drawY = -y / currentScale
      el.style.transform =
        'translateX(' + x + 'px)' + 'translateY(' + y + 'px)' + 'scale(' + currentScale + ')'
    })
    //监听滑动事件
    hammer.on('panright panleft panup pandown', (e) => {
      ...
      this.scale = currentScale
      this.drawX = -translateX / currentScale
      this.drawY = -translateY / currentScale
      el.style.transform =
        'translateX(' +
        translateX +
        'px)' +
